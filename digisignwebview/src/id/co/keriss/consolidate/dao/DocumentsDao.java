...
public class DocumentsDao {
	...
	public DocumentsDao(DB db){
	  ...
	  public Integer findByidDocLock(Long idDoc) throws HibernateException {
		  return (Integer)this.session.createNativeQuery("SELECT * FROM processLockDocument(:idDoc)")
					.setParameter("idDoc", idDoc).getSingleResult();
	  }
	  
	  public Documents findByidDoc(Long idDoc) throws HibernateException {
				  return  (Documents) session.createQuery("from Documents d where d.id="+idDoc).uniqueResult();
	  }
	  
	  public Integer findByidDocUnlock(Long idDoc) throws HibernateException {
		  return (Integer)this.session.createNativeQuery("SELECT * FROM processUnlockDocument(:idDoc)")
					.setParameter("idDoc", idDoc).getSingleResult();
	  }
...
//	    	log.debug(e);
	  ...
	  public  void update(Documents trx) throws Exception {
	    try {
	    	Transaction t = null;
