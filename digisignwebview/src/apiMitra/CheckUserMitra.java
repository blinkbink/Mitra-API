		...
		try{
				...
				// multipart form
					...
					if(token!=null) {
						...
						if(tm!=null) {
							LogSystem.info(request, "Token ada : " + token, refTrx , path_app,  Thread.currentThread().getStackTrace()[1].getLineNumber(), email_req, mitra_req, CATEGORY, (System.currentTimeMillis() - start) / 1000f + "s");
							mitra=tm.getMitra();
						} else {
							LogSystem.error(request, "Token tidak ada", refTrx , path_app,  Thread.currentThread().getStackTrace()[1].getLineNumber(), email_req, mitra_req, CATEGORY, (System.currentTimeMillis() - start) / 1000f + "s");
...
//	         System.out.println("SEND :"+res);
			...
			LogSystem.info(request, jo.toString(), refTrx , path_app,  Thread.currentThread().getStackTrace()[1].getLineNumber(), email_req, mitra_req, "RESPON", (System.currentTimeMillis() - start) / 1000f + "s");
