...
public class UpdateDataUserMitra extends ActionSupport {
	...
	String refTrx="UPD"+sdfDate2.format(tgl).toString();
	String kelas="apiMitra.UpdateDataUserMitra";
...
//			log.error(e);
	...
	JSONObject reqPerubahanData(JSONObject jsonRecv, JPublishContext context, HttpServletRequest  request) throws JSONException{
        ...
        if(aVerf.verification(jsonRecv)){
        	...
        	User user=udao.findByUsernameByMitra(jsonRecv.getString("email").toLowerCase(), uUpdate.getMitra().getId());
        	...
        	if(user!=null) {
        		...
        		if(pdata.size()>0) {
        			...
        			LogSystem.info(request, "Hasil selisih jam = "+check,kelas, refTrx, trxType);
        			if(check<2) {
        				next=false;
        				res="05";
	...
	class MailSender{
		...
		public void run() {
			...
			new SendMailSSL().sendMailKonfPerbData(user, id);
