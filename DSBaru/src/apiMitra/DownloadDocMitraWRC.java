...
public class DownloadDocMitraWRC extends ActionSupport {
	...
	String kelas="apiMitra.DownloadDocMitraWRC";
	String trxType="DWL";
...
//			        FileInputStream inStream = new FileInputStream(downloadFile);
					...
					LogSystem.info(request, "application/pdf download",kelas, refTrx, trxType+"/"+mitra.getName());
					outStream.close();
			        inStream.close();
			        
			        LogSystem.info(request, "success",kelas, refTrx, trxType+"/"+mitra.getName());
				} else {
					context.getResponse().sendError(404);
					LogSystem.info(request, "404",kelas, refTrx, trxType+"/"+mitra.getName());
					return;
				}
...
//				outStream.write(403);
				//outStream.flush();
//				outStream.close();
				LogSystem.info(request, "Akses tidak diijinkan",kelas, refTrx, trxType);
				context.getResponse().sendError(403);
			}
        }
        catch (Exception e) {
