...
public class SignDocMitra extends ActionSupport {
	User userRecv;
	//Date tgl= new Date();
	         ...
	         if(valid==true) {
		         ...
		         try {
					...
					// TODO: handle exception
					 LogSystem.error(request, "System timout",kelas, refTrx, trxType);
		         ...
		         if(dList.get(0).getDocument().getPayment()=='2') {
		        	 ...
		        	 if(balance<jmlttd) {
		        		 LogSystem.info(request, "Balance mitra tidak cukup",kelas, refTrx, trxType);
		        	 ...
		        	 if(inv!=null) {
		        		 ...
		        		 for(DocumentAccess da:dList) {
		        			 try {
		        				 ...
		        				 if(sign==false) {
		        					LogSystem.info(request, "Sign false, Gagal proses tandatangan",kelas, refTrx, trxType);
		        					result.put("result", "06");
					     			result.put("status", status);
								...
								// TODO: handle exception
								LogSystem.info(request, "Gagal proses tandatangan",kelas, refTrx, trxType);
								result.put("result", "06");
				     			result.put("status", status);
				     			result.put("notif", "gagal proses sign");
				     			kp.reverseTransaction(inv);
		        	 ...
		        	 //System.out.println("balance dokumen = "+balance);
					  if(balance<1) {
						  LogSystem.info(request, "Balance doc mitra tidak cukup",kelas, refTrx, trxType);
						  	result.put("result", "06");
			     			result.put("status", status);
					  ...
					  if(li.size()==0) {
						  ...
						  if(inv==null) {
							  LogSystem.info(request, "Gagal proses sign [Bill]",kelas, refTrx, trxType);
							  	result.put("result", "06");
				     			result.put("status", status);
