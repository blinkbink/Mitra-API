...
public class DocumentSigner implements DSAPI{
	...
	// HTTP Post request
	public JSONObject sendingPostRequest(String idAccess, String fileOut, String version, String refTrx) throws JSONException {
		JSONObject res=null;
		try{
	...
	public JSONObject sendingPostRequest2(String idAccess, String fileOut, HttpServletRequest request, String refTrx, String version) throws JSONException {
		...
		try{
			...
			HttpEntity multiPartEntity = builder.build();
			httpPost.setEntity(multiPartEntity);
			httpPost.setConfig(RequestConfig.custom().setConnectionRequestTimeout(30000).build());
			httpPost.setHeader("tokenId", refTrx);
			...
			// TODO Auto-generated catch block
			LogSystem.error(getClass(), e);
			LogSystem.info(request, "Connection refused ke KMS", "DocumentSigning", refTrx, "SGN-KMS");
		}
	...
	public JSONObject kirimKMS(JSONArray idAccess, String fileOut, HttpServletRequest request, String refTrx, Long user, Long documentId, String version) throws JSONException {
		...
		LogSystem.info(request, "URL = "+ url, "DocumentSigning", refTrx, "SGN-KMS");
        JSONObject data=new JSONObject();
        JSONObject req=new JSONObject();
...
//                System.out.println(line);
        	...
        	ex.printStackTrace();
            LogSystem.error(request, ex.toString(), "DocumentSigning", refTrx, "SGN-KMS");
        } catch (KeyManagementException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
			LogSystem.error(request, e.toString(), "DocumentSigning", refTrx, "SGN-KMS");
	...
	public JSONObject kirimMeterai(HttpServletRequest request, String refTrx, Long documentId, String docName, Long mitra, float llx, float lly, float urx, float ury,int page, String document, String location) {
		...
		LogSystem.info(request, "URL = "+ url, "DocumentSigningMaterai", refTrx, "SGN-METERAI");
        JSONObject data=new JSONObject();
        JSONObject req=new JSONObject();
        String charset = "UTF-8";
		try {
			...
			LogSystem.info(request, "isi data proses ke Meterai = "+StringUtils.left(data.toString(), 200), "DocumentSigningMaterai", refTrx, "SGN-METERAI");
		...
		try {
            ...
            LogSystem.info(request, "Response code "+ conn.getResponseCode(), "DocumentSigningMaterai", refTrx, "SGN-METERAI");
