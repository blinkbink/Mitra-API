...
public class FileProcessor implements DSAPI{
	  ...
	  Date tgl= new Date();
		...
		String kelas="util.FileProcessor";
	...
	public FileProcessor(String refTrx) {
		this.refTrx=refTrx;
	}
...
//      Blob blob = Hibernate.createBlob(fileSave.getInputStream());
      */
      ...
      try {  
    	    ...
    	    LogSystem.info(request, "jumlah page = "+jmlPage, kelas, refTrx, trxType);
    	    LogSystem.info(request, "size text = "+textFromPdfFilePageOne.length(), kelas, refTrx, trxType);
    	}  
    	catch ( Exception e ) {
    	    // handle exception
    		//fileTo.delete();
	...
	public JSONObject uploadFILEnQRMitra2(HttpServletRequest  request,DB db, User userTrx, FileItem fileSave, String idMitra, JSONObject message) throws Exception {
		  String uploadTo =  "/file2/data-DS/UploadFile/"+userTrx.getId()+"/original/";
		  String directoryName = "/file2/data-DS/UploadFile/"+userTrx.getId()+"/original/";
		...
		if (fileSave == null)
		{
			LogSystem.error(request, "Filesave " + fileSave, kelas, refTrx, trxType);
			res.put("rc", "01");
	        res.put("keterangan", "Kirim dokumen gagal");
	        return res;
		}
...
//	    	ddao.updateWhere(idMitra, userTrx.getMitra().getId());
			...
			LogSystem.error(request, "DB Failed", kelas, refTrx, trxType);
	  		...
	  		throw new Exception("Failed save data document");
		}
