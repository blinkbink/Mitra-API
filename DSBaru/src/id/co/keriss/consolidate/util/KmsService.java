...
//			jo.put("JSONFile", KMSRSAEncryption.encryptWithPub(jf.toString()));
	...
	public String createSertifikat(Long eeuser) {
...
//			System.out.print(jo);
			...
			LogSystem.info(request, "Response create sertifikat : " + rst.toString(), kelas, kodeTrx, trxType);
		} catch (Exception e) {
			e.printStackTrace();
			LogSystem.error(request, e.getMessage(), kelas, kodeTrx, trxType);
		}
...
//			jo.put("JSONFile", KMSRSAEncryption.encryptWithPub(jf.toString()));
			...
			LogSystem.info(request, "Response revoke sertifikat : " + rst.toString(), kelas, kodeTrx, trxType);
		} catch (Exception e) {
			LogSystem.error(request, e.getMessage(), kelas, kodeTrx, trxType);
		}
		return res;		
...
//			String dataResp = KMSRSAEncryption.decryptWithPub(res1.getString("JS/ONFile"));
			res = new JSONObject(dataResp);
			//log.info("[DATA] " + dataResp);
			LogSystem.info(request, res.toString(), kelas, kodeTrx, trxType);
			response = res.getString("result");
