...
public class DocumentsAccessDao {
	...
	public DocumentsAccessDao(DB db){
	  ...
	  public List<DocumentAccess> findDocAccessMeterai(Long idDoc) throws HibernateException {
		  String  qString="select da "
		  					...
		  					+ "where "
		  						...
		  						+ "(da.type = 'meterai') order by da.id asc";
	  ...
	  public List<DocumentAccess> findByDocMeterai(String document) throws HibernateException {
		    return  session.createQuery("from DocumentAccess d where d.document ='"+document+"' and type='meterai' order by d.id desc").list();
	  }
	  public List<DocumentAccess> findDocMeteraiSignApp(Long document) throws HibernateException {
		  //String  qString="select d from DocumentAccess d where d.document =:doc and  d.email=:email and (type='sign' or type='initials') and flag='false'";
		  String  qString="select d from DocumentAccess d where d.document =:doc and type='meterai'";
		  Query query=session.createQuery(qString);
	  ...
	  public Long getWaitingSignUser(User u) throws HibernateException {
		  return (Long) session.createQuery("select count(id) from DocumentAccess where (type='sign' or type='initials') and flag=false and page!= 0 and  (userdata='"+u.getUserdata().getId()+"' or email='"+u.getNick()+"')").uniqueResult();
	     
	  }
	  
	  public Long getWaitingSignUserByDoc(String doc) throws HibernateException {
		  return (Long) session.createQuery("select count(id) from DocumentAccess where (type='sign' or type='initials') and flag=false and document='"+doc+"'").uniqueResult();
	  ...
	  public List<DocumentAccess> findDocEEuserSign(Long document, String email) throws HibernateException {
		  String  qString="select d from DocumentAccess d where d.document =:doc and d.email=:email and (type='sign' or type='initials') and flag='false' order by d.id asc";
		  Query query=session.createQuery(qString);
	  ...
	  public List<DocumentAccess> findByEmail(String email) throws HibernateException {
		  String  qString="select d from DocumentAccess d where d.email=:email and d.eeuser is null";
		  Query query=session.createQuery(qString);
	  ...
	  public List<DocumentAccess> findDocEEuserSigned(Long document, String email) throws HibernateException {
	      String  qString="select d from DocumentAccess d where d.document =:doc and  d.email=:email and (type='sign' or type='initials') and flag='true'";
	      Query query=session.createQuery(qString);
	  ...
	  public boolean checkSignLocation(Long document) throws HibernateException {
		     ...
		     query.setLong("doc", document);
		     List <DocumentAccess> dataRow=query.list();
		     if(dataRow.size()==0) {
		    	 result=false;
		     }
		     if(dataRow.size()>0) {
	  ...
	  public List<DocumentAccess> findByDocInv(Long doc) throws HibernateException {
		  String  qString="select d from DocumentAccess d where d.document=:doc and d.invoice is not null and d.invoice!=''";
		  Query query=session.createQuery(qString);
		  
		  query.setLong("doc", doc);  
		    
		  return query.list();
	  }
	  
	  public List<DocumentAccess> findByDocFlagTrue(Long doc) throws HibernateException {
		  String  qString="select d from DocumentAccess d where d.document=:doc and d.flag=true and d.invoice is not null";
		  Query query=session.createQuery(qString);
...
//			    	log.debug(e);
	  ...
	  public void updateWhereDocumentEmail(Long doc, String email) {
			...
			try {
			      ...
			      String  qString="update DocumentAccess "
							+ "set "
								+ "flag=false "
							+ "where "
								+ "email=:email and document=:document";
					Query query=session.createQuery(qString);
					query.setLong("document", doc);
					query.setString("email", email);
					query.executeUpdate();
...
//			    	log.debug(e);
	  ...
	  public void updateWhereDocumentEmailFlagTrue(Long doc, String email, Date date, String inv) {
		  
			try {
			      Transaction t=session.beginTransaction();
			      String  qString="update DocumentAccess "
							+ "set "
								+ "flag=true, date_sign=:date, invoice=:inv "
							+ "where "
								+ "email=:email and document=:document and flag=false";
					Query query=session.createQuery(qString);
					query.setLong("document", doc);
					query.setString("email", email);
