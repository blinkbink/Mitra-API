...
//			System.out.println(msgResp.toString());
	...
	public JSONObject checkFaceWithKTP(byte [] ktp, byte [] wajah, Long idmitra) {
		 JSONObject msgResp=new JSONObject();
	        try {
				msgResp.put("connection", false);
				msgResp.put("score", 1.00);
...
//			System.out.println(msgResp.toString());
	 ...
	 * @return
	 */
	public JSONObject registerFace(byte [] wajah, String nik) {
		 JSONObject msgResp=new JSONObject();
	        try {
				msgResp.put("result", false);
		        msgResp.put("info", "Internal error");
			} catch (JSONException e1) {
				// TODO Auto-generated catch block
				e1.printStackTrace();
			}
...
//			}
			...
			try {
				while(tryCnt<4 && msgResp.getBoolean("connection")==false) {
					...
					try {
						if(tryCnt>1) {
							obj = new URL(FACE_API_WEFIE+"/register");
						}else {
							obj = new URL(FACE_API_WEFIE+"/register");
						}
					...
					con = (HttpsURLConnection) obj.openConnection();
						...
						Map<String,String> arguments = new HashMap<>();
						arguments.put("face", new String(wajah, StandardCharsets.US_ASCII));
						...
						if(responseCode==200) {
							try {
								...
								// TODO Auto-generated catch block
								e1.printStackTrace();
								log.error("FACE REGISTRATION", e1);
							}
						}
						else  {
							try {
								log.info("Response != masuk 200");
							    msgResp.put("info", "Internal Error");
								msgResp.put("connection", false);
							} catch (JSONException e1) {
								// TODO Auto-generated catch block
						...
						try {
							...
							if(rsJson.getInt("rc")==10)msgResp.put("result", true);
							else msgResp.put("result", false);
						...
						log.error("FACE REGISTRATION", e2);
						try {
							msgResp.put("connection", false);
						} catch (JSONException e1) {
							// TODO Auto-generated catch block
...
//			System.out.println(msgResp.toString());
	...
	public JSONObject findFace(byte [] wajah, String nikUser, String nikVerifikator) {
		 JSONObject msgResp=new JSONObject();
		 try {
				msgResp.put("result", false);
				msgResp.put("RC", "A1");
		        msgResp.put("info", "verifikator tidak terdaftar");
		        JSONObject res=findFace(wajah, nikVerifikator);
		        if(res.getBoolean("result")) {
		        	res=findFace(wajah, nikUser);
			        if(res.getBoolean("result")) {
						msgResp.put("result", true);
						msgResp.put("RC", "00");

		        	}else {
				        msgResp.put("info",res.getString("message"));
						msgResp.put("RC", "12");
	 ...
	 * @return
	 */
	private JSONObject findFace(byte [] wajah, String nik) {
		 JSONObject msgResp=new JSONObject();
	        try {
				msgResp.put("result", false);
		        msgResp.put("info", "verfikasi gagal, silakan coba kembali");
...
//			}
			...
			try {
				while(tryCnt<4 && msgResp.getBoolean("connection")==false) {
					...
					try {
						if(tryCnt>1) {
							obj = new URL(FACE_API_WEFIE+"/find_fcn");
						}else {
							obj = new URL(FACE_API_WEFIE+"/find_fcn");
						}
					...
					con = (HttpsURLConnection) obj.openConnection();
						...
						if(responseCode==200) {
							try {
								...
								// TODO Auto-generated catch block
								e1.printStackTrace();
								log.error("FACE VERIFICATION", e1);
							}
						}
						else  {
							try {
						...
						try {
							...
							if(rsJson.getInt("rc")==10 && rsJson.getBoolean("nik_face"))msgResp.put("result", true);
							else msgResp.put("result", false);
					        msgResp.put("info", rsJson.getString("message"));
						...
						log.error("FACE VERIFICATION", e2);
						try {
							msgResp.put("connection", false);
						} catch (JSONException e1) {
							// TODO Auto-generated catch block
